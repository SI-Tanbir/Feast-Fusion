...
const {refetch}=useCart()
  ...
  const handleCart = (data) => {
    ...
    if (userEmail && userEmail.email) {
      ...
      axios
        .post("http://localhost:5000/carts", payload)
        .then((res) => {
          console.log(res);
          if (res.status == 200) {
            Swal.fire({
      ...
      <div className="navbar bg-base-100 text-center justify-center">
        ...
        <div className="navbar-center hidden lg:flex">
          <ul className="menu menu-horizontal px-1">
            <li>
              <button onClick={() => handledata("salad")}>SALAD</button>
            </li>
            <li>
              <button onClick={() => handledata("pizza")}>PIZZA</button>
            </li>
            <li>
              <button onClick={() => handledata("soup")}>SOUPS</button>
            </li>
            <li>
              <button onClick={() => handledata("dessert")}>DESSERTS</button>
            </li>
            <li>
              <button onClick={() => handledata("drinks")}>DRINKS</button>
            </li>
          </ul>
        </div>
      </div>
